local function create_commit_command_mappings(prefix)
    ...
    for key, val in pairs(commitCommandsShort) do
        ...
        vim.keymap.set(
            'n',
            '<Plug>(git-rebase-n-use-command-' .. val .. ')',
            '^WWBBciw' .. val .. '<Esc>W' .. [[:silent call repeat#set("\<Plug>(git-rebase-n-use-command-]] .. val .. ')")<CR>',
            opt
        )
        vim.keymap.set('x', '<Plug>(git-rebase-x-use-command-' .. val .. ')', ':normal! ^WWBBciw' .. val .. '<Esc><CR>`<', opt)
        ...
        vim.keymap.set('n', prefix .. key, '<Plug>(git-rebase-n-use-command-' .. val .. ')', { buffer = true })
        vim.keymap.set('x', prefix .. key, '<Plug>(git-rebase-x-use-command-' .. val .. ')', { buffer = true })
    end
